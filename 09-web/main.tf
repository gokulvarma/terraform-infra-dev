...
module "web"{
    ...
    vpc_security_group_ids = [data.aws_ssm_parameter.web-security_group_id.value]
...
resource "aws_ec2_instance_state" "stop-web" {
  instance_id = module.web.id
  state       = "stopped"
   depends_on = [
    null_resource.web
...
resource "aws_ami_from_instance" "web" {
  ...
  depends_on = [
    aws_ec2_instance_state.stop-web
  ]
}
